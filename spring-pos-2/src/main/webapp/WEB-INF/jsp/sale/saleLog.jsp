			...
			<div class="table-title">
				<div class="form-inline justify-content-end"
					...
					<button type="button" class="btn btn-primary ml-3" id="reset"
						onclick="location.href = '/pos/sale/saleLog'"
					...
					<div class="form-group ml-3">
						<select class="form-control" id="sortCriteria">
							...
							<option value="saleDate">판매날짜</option>
							<option value="saleTotalPrice">판매금액</option>
						</select>
					...
					<div class="form-group ml-3">
						<select class="form-control" id="searchCriteria" name='searchCriteria'
							...
							<option value="saleProducts">상품코드</option>
							<option value="saleProducts">상품명</option>
							<option value="saleDate">판매날짜</option>
						</select>
					...
					<table class="table table-bordered">
						<thead>
							<tr>
								...
								<th>받은돈</th>
								<th>거스름돈</th>
								<th  style="width:25%;">취소날짜</th>
							</tr>
						</thead>
						<tbody>
							<c:forEach items="${list}" var="sale">
								<tr onclick=" openPopup('/pos/sale/saleView?saleNo=${sale.saleNo}', '판매 내역', 1030,900 )">
									<c:choose>
										<c:when test="${sale.saleCanceled != null}">
											<td><label class="btn btn-danger">취소</label>${sale.saleNo}</td>
										</c:when>
										<c:otherwise>
											<td>${sale.saleNo}</td>
										</c:otherwise>
									...
									<td>${sale.saleDate}</td>
									<td>${sale.saleTotalPrice}원</td>
									<td>${sale.saleReceived}원</td>
									<td>${sale.saleChanged}원</td>
									<td>${sale.saleCanceled != null ? sale.saleCanceled : "-"}</td>
